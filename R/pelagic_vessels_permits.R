#' List of vessels with pelagic fishing permits
#'
#' A `data.frame` object extracted from a raw dataset of permits available
#' under request at dataMares (https://datamares.org/)
#'
#' @format A `data.frame` with 719 rows and 2 columns.
#' \describe{
#'   \item{RNP}{Unique code identifying the vessel}
#'   \item{vessel_name}{Name of the vessel}
#'   ...
#' }
#' @source \url{https://www.datamares.org/}

"pelagic_vessels_permits"
